...
1. Browsers first apply the selector as if it were called on the whole document, producing a list of globally matched candidates. 
2. Then they filter that list to keep only those elements that are descendants of the `baseElement`.
3. Then the first match of those remaining elements is returned.

Note: `element.querySelectorAll()` also applies selector scoping in the same manner.

## How do you fix it?

If we want to restrict the selector to the element on which it is called, we should use the [`:scope` pseudo-class](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Selectors/:scope).

```js
const strictQuery = baseElement.querySelector(':scope div span');
// Returns null, as expected
```

As mentioned on MDN, this works because:
