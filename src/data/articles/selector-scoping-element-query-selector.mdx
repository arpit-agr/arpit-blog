---
title: Selector scoping in element.querySelector() and element.querySelectorAll()
...
If we want to restrict the selector to the element on which it is called then we should use the [`:scope` pseudo-class](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Selectors/:scope).
